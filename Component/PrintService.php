...
class PrintService
{
    ...
    private function addCoordinates()
    {
        ...
        $pdf->Text(
            $this->conf['fields']['extent_ur_y']['x'] + $corrFactor,
        ...
        $pdf->TextWithDirection(
            $this->conf['fields']['extent_ur_x']['x'] + 1,
            $this->conf['fields']['extent_ur_x']['y'],
            round($this->data['extent_feature'][2]['x'], $precision),
            'D'
        );
        ...
        $pdf->Text(
            $this->conf['fields']['extent_ll_y']['x'],
        ...
        $pdf->TextWithDirection(
            $this->conf['fields']['extent_ll_x']['x'] + 3,
            $this->conf['fields']['extent_ll_x']['y'] + 30,
            round($this->data['extent_feature'][0]['x'], $precision),
            'U'
        );
    }
    ...
    private function addDynamicText()
    {
        ...
        $this->pdf->MultiCell(
            $this->conf['fields']['dynamic_text']['width'],
        ...
        foreach ($this->data['legends'] as $idx => $legendArray) {
            ...
            foreach ($legendArray as $title => $legendUrl) {
                ...
                if ($c > 1) {
                    ...
                    } elseif (($y - $yStartPosition) + $tempY + 10 > $height && $legendConf == true) {
                        $this->pdf->addPage('P');
                        $x = 5;
                        $y = 10;
                        $legendConf = false;
                ...
                if ($legendConf == true) {
                    ...
                    $this->pdf->setXY($x, $y);
                    $this->pdf->Cell(0, 0, utf8_decode($title));
                    $this->pdf->Image($image, $x, $y + 5, ($size[0] * 25.4 / 96), ($size[1] * 25.4 / 96), 'png', '',
                        false, 0);
